...
//: 추성 메서드와 상수만으로 구성된 특수한 클래스
//- 모든 메서드가 추상 메서드인 클래스와 유사
//- 객체의 기능을 정의하고, 실제 구현은 인터페이스를 구현하는 클래스에서 담당

//인터페이스의 필요성
//: 서로 다른 클래스에 댛 공통된 동작을 보장할 때
//	 - 인터페이스를 구현하는 모든 클래스는 인터페이스의 모든 메서드를 반드시 구현
//: 여러 인터페이스를 동시에 구현 가능
//	 - 한 클래스에 대해 여러 개의 인터페이스를 구현 가능
...
//: interface는 접근제어자로 public, defalut만 사용 가능
...
interface PlayingCard {
	// 멤버 변수(public static final 포함	// 생략 가능하다는 말)
	public static final int SPADE = 4;
	final int DIAMOND = 3;
	static int HEART = 2;
	int CLOVER = 1;
	
	// 메서드 (public abstract 포함		// 생략 가능하다는 말)
	public abstract String getCardNumber();
	
	String getCardKind();
	
	// 디폴트 메서드
	// : 인터페이스에 새 기능을 축가하면서 기존 구현을 포함
	...
	// 정적 메서드
	// : 인터페이스에서 직접 구현 가능
	// - 인터페이스 이름으로 직접 호출 가능, 오버라이딩(재정의) 불가능
...
//인터페이스의 구현
//: 클래스의 implements 키워드를 사용하여 인터페이스 구현
